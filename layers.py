...
class dynamic_routing(torch.nn.Module):
    ...
    def forward(self,x):
        ...
        batch_size = x.size(0)
        W = torch.cat([self.W] * batch_size, dim=0) # map matrix W
        representation = torch.matmul(x, W) # U = WX, representation
        r_sum = torch.sum(representation, dim=-1, keepdim=False)
        b = torch.zeros([batch_size, num_nodes]) #Returns a tensor filled with the scalar value 0
        b = Variable(b)
        one = torch.ones_like(r_sum) # 返回一个填充了标量值1的张量，其大小与之相同 input
        zero = torch.zeros_like(r_sum) # 0
        label = torch.clone(r_sum)
        label = torch.where(label == 0, one, zero)
        b.data.masked_fill_(label.bool(), -float('inf'))
        num_iterations = 3 #迭代次数 r
