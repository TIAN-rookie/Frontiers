...
class CasSeqGCN(torch.nn.Module):
    ...
    def _setup_GCN_layers(self):
        ...
        self.GCN_layers = my_GCN(in_channels=self.number_of_features, # 2 ?
                                 out_channels=self.args.gcn_out_channel, #32
                                 filters_1=self.args.gcn_filters_1, #32
                                 filters_2=self.args.gcn_filters_2, #32
                                 dropout=self.args.gcn_dropout) #0.1
    ...
    def forward(self, data):
        ...
        for sub_graph in data:
            ...
            FN_features = sub_graph['FN_fea']
            FN_representations.append(FN_features)
            gcn_representation = torch.nn.functional.relu(self.GCN_layers(edges, features)) # edges, features
            hidden_representations.append(gcn_representation)
        hidden_representations = torch.cat(tuple(hidden_representations))
        FN_representations = torch.cat(tuple(FN_representations))
        ...
        '''
        FN_features = create_FN_fratures()
        sub_graph_representation = torch.cat((sub_graph_representation, FN_representation), dim = 1)
        
        '''

        sub_graph_representation = torch.cat((sub_graph_representation, FN_representations), 1)
...
class CasSeqGCNTrainer(object):
    ...
    def enumerate_unique_labels_and_targets(self):
        ...
        self.number_of_features = len(self.feature_map) # 2?
    ...
    def create_data_dictionary(self, edges, features, FN_fea):
        """
    ...
    def CreateGraph(self):
        #graph_folder = '/Users/tianchuhe/PycharmProjects/CasSeqGCN/weibo/weibo_all_batch_5_10_100_dataset_9hours/'
        graph_folder = '/Users/tianchuhe/PycharmProjects/CasSeqGCN/synthetic_data_V2/'
        ending = "*.json"
        graph_paths = sorted(glob.glob(graph_folder + ending), key=os.path.getmtime)
        self.Graph = nx.DiGraph()
        for path in tqdm(graph_paths):
            data = json.load(open(path))
            data_dict = data
            edge_list = data['graph_info']['edges']
            self.Graph.add_edges_from(edge_list)


    def Find_FN(G, node_list):
        FN_list = []
        for node in node_list:
            if node in G:
                for FNs in list(nx.neighbors(G, node)):
                    if (FNs not in node_list):
                        FN_list.append(FNs)
    ...
    def create_features(self, data):
        ...
        for k in data['labels'].keys():
            if data['labels'][k] == 1.0:
                Node_list = [(int(k))]
                FN_list = []
                for node in Node_list:
                    if node in self.Graph:
                        for FNs in list(nx.neighbors(self.Graph, node)):
                            if (FNs not in Node_list):
                                FN_list.append(FNs)
                FN_list = list(set(FN_list))
            
                #value = self.Find_FN(self.G, Node_list)
                FN_label[k] = len(FN_list)
            else:
                FN_label[k] = 0
        x = []
        for n in FN_label.values():
            x.append(n)
        ...
        '''
        features[node_indices, self.number_of_features - 2] = 1.0
        #features[node_indices, feature_indices] = 1.0
        for node_index in node_indices:
            features[node_index, self.number_of_features - 1] = z[str(self.nodes_map[node_index])]
            #features[node_index, self.number_of_features] = self.in_degree[str(self.nodes_map[node_index])]
            features[node_index, self.number_of_features] = self.in_degree[str(self.nodes_map[node_index])]
            features[node_index, self.number_of_features + 1] = self.out_degree[str(self.nodes_map[node_index])]
    ...
    def create_FN_fea(self, data):
        ...
        for k in data['labels'].keys():
            if data['labels'][k] == 1.0:
                ...
                FN_label[k] = 0
        KV = len(Node_list)
        KF = len(data['labels']) - KV
        KF_non = len(FN_list)
        FN_fea[0, 0] = 1.0
        FN_fea[0, 1] = KV
        FN_fea[0, 2] = KF
        FN_fea[0, 3] = KF_non
        FN_fea = torch.FloatTensor(FN_fea)
        FN_fea = F.normalize(FN_fea, p=2, dim=1)
    ...
    def create_input_data(self, path):
        ...
        for i in range(0, len(data) - self.args.sub_size):
            ...
            FN_fea = self.create_FN_fea(data[graph_num]) #
            to_pass_forward.append(self.create_data_dictionary(edges, features, FN_fea))
